...
_ui_setup() {
  if [[ -t 1 && "${NO_COLOR:-}" != "1" ]]; then
    ...
    if [[ "${COLORTERM:-}" == *truecolor* || "${COLORTERM:-}" == *24bit* || "${TERM:-}" == *-direct ]]; then
      ...
      UI_YELLOW=$'[38;2;249;226;175m'  # yellow
      UI_BLUE=$'[38;2;137;180;250m'    # blue
      UI_MAGENTA=$'[38;2;203;166;247m' # mauve
      UI_CYAN=$'[38;2;137;220;235m'    # sky
      UI_WHITE=$'[38;2;205;214;244m'   # text
      UI_PEACH=$'[38;2;250;179;135m'
      UI_TEAL=$'[38;2;148;226;213m'
      UI_PINK=$'[38;2;245;194;231m'
    else
...
compose_engine() {
  # Low-level compose runner for the selected engine (from .env CONTAINER_ENGINE).
  ...
  case "${CONTAINER_ENGINE}" in
    podman)
      if command -v podman-compose >/dev/null 2>&1; then
        ...
        if podman ps --filter "status=running" \
...
# Include data path keys so a snapshot from another layout (e.g. Immich's ./volume/*)
# cannot point compose at empty dirs while restore writes under ./data/*.
_HOST_INSTALL_ENV_KEYS=(
  CONTAINER_ENGINE IMMICH_PORT HTTP_PORT PORT NEXTCLOUD_PORT COLLABORA_PORT
  UPLOAD_LOCATION DB_DATA_LOCATION MODEL_CACHE_LOCATION REDIS_DATA_LOCATION
)

resolve_repo_path() {
  # resolve_repo_path REL_OR_ABS ? absolute path under ROOT when relative.
  local p="${1:-}"
  [[ -n "${p}" ]] || return 1
  if [[ "${p}" == /* ]]; then
    printf '%s\n' "${p}"
